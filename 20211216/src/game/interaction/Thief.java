...
public class Thief {
	...
	// Thief쪽에서 Orc쪽의 수치를 깎아주고
	// Orc쪽에서 Thief의 수치를 깎아야 합니다.
	public void huntOrc(Orc orc) {
		// 도둑이 오크를 때리기 위해서 먼저 orc의 doBattle을 호출합니다.
		orc.doBattle(this.atk);
		// 도둑도 오크에게 공격을 받습니다.
		this.hp = this.hp + this.def - orc.getAtk();
		System.out.println("교전 결과 도둑의 체력 : " + this.hp);
	}
	
	public void huntTroll(Troll troll) {
		troll.doBattle(this.atk);// 트롤을 도둑이 공격하는 로직
		...
		System.out.println("교전 결과 도둑의 체력 : " + this.hp);//교전 결과 브리핑
	}
	
	public void huntDragon(Dragon dragon) {
		dragon.doBattle(this.atk);// 드래곤을 도둑이 공격하는 로직
	...
	//getter/setter를 Thief, Orc에 만들어주세요.
	public int getHp() {
		return hp;
	}
